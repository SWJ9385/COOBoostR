...
MutTo1mb <- function(bedpath, resultpath, mb1_path, bed_sep = ",", bed_header = F, bedform = "bed4"){
  ...
  if (bedform == "bed4"){
    ...
    for (j in file_list){
      ...
      write.table(mb1, paste0(resultpath,j,"_mut_converted.csv"), row.names = F, col.names = F, quote = F, sep = ",")
